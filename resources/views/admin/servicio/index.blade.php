                ...
                <div class="p-6">
                    <div class="container mx-auto p-4">
                        ...
                        <div class="bg-gray-700 rounded-lg shadow-xl p-6">
                            <h1 class="text-3xl font-bold mb-6 text-center text-blue-400">Listado de Servicios</h1>
                            ...
                            <div class="overflow-x-auto">
                                <table class="min-w-full leading-normal text-white">
                                    ...
                                    <tr class="bg-gray-600 text-white">
                                        ...
                                        <th class="px-5 py-3 border-b-2 border-gray-500 text-left text-xs font-semibold uppercase tracking-wider">Ícono</th>
                                        <th class="px-5 py-3 border-b-2 border-gray-500 text-left text-xs font-semibold uppercase tracking-wider">Título</th>
                                        <th class="px-5 py-3 border-b-2 border-gray-500 text-left text-xs font-semibold uppercase tracking-wider">Descripción</th>
                                        ...
                                        <tr class="hover:bg-gray-600 transition duration-300 ease-in-out">
                                            ...
                                            <td class="px-5 py-5 border-b border-gray-500 text-sm">
                                                @if ($servicio->imagen_icono && file_exists(public_path('storage/' . $servicio->imagen_icono)))
                                                    <img src="{{ asset('storage/' . $servicio->imagen_icono) }}" alt="Ícono" class="w-10 h-10 object-contain rounded-md border border-gray-400">
                                                @else
                                                    <div class="w-10 h-10 bg-white border border-gray-400 rounded-md flex items-center justify-center text-xs text-gray-600">
                                                        Sin imagen
                                                    </div>
                                                @endif
                                            </td>
                                            ...
                                            <td class="px-5 py-5 border-b border-gray-500 text-sm">
                                                {{ $servicio->datoPersonal->nombre ?? 'Sin dato' }}
                                            </td>
                                            <td class="px-5 py-5 border-b border-gray-500 text-sm">
                                                <div class="flex flex-wrap gap-2">
                                                    <a href="{{ route('admin.servicio.show', $servicio->id) }}"
                                                       class="bg-green-600 hover:bg-green-700 text-white font-semibold px-4 py-2 rounded-md shadow-sm transition duration-200">
                                                        👁 Ver
                                                    </a>
                                                    <a href="{{ route('admin.servicio.edit', $servicio->id) }}"
                                                       class="bg-yellow-500 hover:bg-yellow-600 text-white font-semibold px-4 py-2 rounded-md shadow-sm transition duration-200">
                                                        ✏️ Editar
                                                    </a>
                                                    <form action="{{ route('admin.servicio.destroy', $servicio->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de que quieres eliminar este servicio?');">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit"
